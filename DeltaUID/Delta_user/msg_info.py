...
class MsgInfo:
    ...
    async def _get_user_collections(
        ...
        for i, result in enumerate(collection_results):
            if isinstance(result, Exception):
                logger.error(f"获取藏品信息异常: {result!s}")
    ...
    async def get_msg_info(self) -> Union[InfoData, str]:
        ...
        for result, name in zip(
            ...
            if isinstance(result, Exception):
                logger.error(f"获取{name}失败: {result!s}")
        ...
        if not all([player_info_res["status"], sol_info["status"], tdm_info["status"]]):
            # 检查是否是登录过期
            if (
                player_info_res.get("login_expired")
                or sol_info.get("login_expired")
                or tdm_info.get("login_expired")
            ):
                return ERROR_LOGIN_EXPIRED
            return ERROR_SERVER_BUSY
    ...
    async def get_depot_red_info(self):
        ...
        try:
            ...
            logger.error(f"读取item.json失败: {e!s}")
            return None
        data_list = data["list"]
